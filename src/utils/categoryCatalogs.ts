 ...
 *
 * The category array is source-agnostic - both TPDB and StashDB use the same
 * curated list. The Go backend owns the manifest and catalog IDs; this edge
 * only validates the configured category slugs.
 */
...
const CATEGORIES = [
  ...
  { slug: 'ebony',              name: 'Ebony',              stashTag: 'Ebony',              tpdbQuery: 'Ebony',              default: false },
...
/**
 * StashDB catalog categories with sparse coverage (audit 2026-06-30): Petite
 * (0), Ebony (3), Pissing (8) streamable scenes in enriched_scenes. Surfaced
 * as a note next to these options in the StashDB configure panel so users know
 * to expect few results before opting in. Static - these are genuine data
 * sparseness, not alias mismatches; re-audit (cmd/tpbaudit) if StashDB coverage
 * shifts.
 */
const STASHDB_LOW_RESULT_SLUGS = new Set(['petite', 'ebony', 'pissing']);

function isStashdbLowResult(slug: string) {
  return STASHDB_LOW_RESULT_SLUGS.has(slug);
}

export { CATEGORIES, getCategories, getCategoryBySlug, defaultEnabledSlugs, allSlugs, nameToSlug, STASHDB_LOW_RESULT_SLUGS, isStashdbLowResult, };
