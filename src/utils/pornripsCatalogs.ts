...
/**
 * When hideFromHome is enabled, catalogs need a required `genre` extra so Stremio's
 * Home/Board skips them. If the catalog already declares a `genre` dropdown,
 * mark that one required - do not append a second `genre` (Stremio honours the
 * first optional genre and still loads the row on Home).
 */
...
/**
 * Build the Stremio manifest catalog entries for PornRips.
 * @param {Set<string>|null} disabled - pr_ ids to omit
 * @param {Array} homeGenre - extra appended for hideFromHome (may be empty)
 * @param {string[]} disabledPrStudios - studio names to omit from the Studio dropdown
 */
function getPornripsManifestCatalogs(disabled: Set<string> | null | undefined, homeGenre: any[] = [], disabledPrStudios: string[] = []) {
  const disabledStudioSet = new Set(disabledPrStudios || []);
  return PR_CATALOGS
    ...
    .map((c) => {
      ...
      if (c.genre) {
        ...
        if (c.id === 'pr_studio') {
          options = options.filter((s) => !disabledStudioSet.has(s));
        }
        // hideFromHome catalogs force a required genre so Stremio's Home skips
        // them (Discover auto-selects 'All' and lets the user expand the dropdown).
        extra.push({ name: 'genre', options: ['All', ...options], ...(c.hideFromHome ? { isRequired: true } : {}) });
      }
...
export { PORNRIPS_STUDIOS, PORNRIPS_TAGS, PORNRIPS_QUALITIES, PR_CATALOGS, getPornripsBases, getPornripsManifestCatalogs, getPornripsParams, dedupePornrips, };
