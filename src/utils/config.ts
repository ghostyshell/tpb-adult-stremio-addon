 ...
 *
 * Config fields:
 *   rdKey            - Real-Debrid API key   (mutually exclusive with tbKey / pmKey)
 *   tbKey            - TorBox API key        (mutually exclusive with rdKey / pmKey)
...
// is read-only and requires an account (free invite via the StashDB access guide:
...
const DEFAULT_CONFIG: any = {
  ...
  // manifest id/name also carry the provider (rd/tb/pm) so the per-provider
...
function parseConfig(encoded?: string): AddonConfig {
  ...
  {
    const VALID = new Set(['piratebay', 'pornrips', 'hentai', 'sukebei', 'stripchat', 'perverzija', 'freepornvideos', 'yesporn', 'watchporn', 'hqporner']);
