 ...
 * keeps only stream resolution (12 debrid providers + client IP forwarding),
...
/**
 * Resolve the public base URL for this addon instance. Prefers an explicit
 * PUBLIC_HOST env var (e.g. "https://tpb-adult-addon.click") so a spoofed
 * X-Forwarded-Host cannot poison install URLs or the Go backend's
 * X-Addon-Base-Url (which gets baked into manifest poster/proxy URLs).
 * Falls back to the request's own host when unset.
 */
function publicBaseUrl(req: Request): string {
  const pinned = (process.env.PUBLIC_HOST || '').trim().replace(/\/$/, '');
...
async function proxyStremioToGo(req: Request, res: Response, subpath: string, configOverride?: string) {
  ...
  const headers = {
    ...
    'X-Addon-Base-Url': publicBaseUrl(req),
