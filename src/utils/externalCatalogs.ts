...
const STRIPCHAT_CATALOGS = [
  { id: 'sc_girls', name: 'Stripchat · Girls' },
  { id: 'sc_couples', name: 'Stripchat · Couples' },
  { id: 'sc_guys', name: 'Stripchat · Guys' },
  { id: 'sc_trans', name: 'Stripchat · Trans' },
];
...
function getStripchatManifestCatalogs(disabled: Set<string> | null | undefined, homeGenre: any[] = []) {
  const opt = { isRequired: false };
  return STRIPCHAT_CATALOGS
    .filter((c) => !(disabled && (disabled.has(c.id) || disabled.has('stripchat'))))
    .map((c) => ({
      type: 'Porn',
      id: c.id,
      name: c.name,
...
export { PORNTUBE_CATALOGS, HENTAI_CATALOGS, SUKEBEI_CATALOGS, STRIPCHAT_CATALOGS, getPorntubeManifestCatalogs, getHentaiManifestCatalogs, getSukebeiManifestCatalogs, getStripchatManifestCatalogs, getPorntubeBases, getHentaiBases, porntubeTheirCatalogId, hentaiTheirCatalogId, };
