...
async function addTorrentUrl(cookie: string, torrentUrl: string) {
  ...
  const entry = Object.values(result as Record<string, { torrentid?: string } | undefined>)
    .find((v) => v && typeof v === 'object' && v.torrentid);
...
async function resolveStreams(creds: string, infoHash: string, magnetLink: string) {
  const cacheKey = `ls:files:${scopeKey(creds)}:${infoHash}`;
  return cachedResolve(streamCache, cacheKey, () => resolveSrc(creds, magnetLink, true));
}

async function resolveStreamsFromTorrentFile(creds: string, torrentUrl: string) {
  const cacheKey = `ls:files-turl:${scopeKey(creds)}:${torrentUrl}`;
  return cachedResolve(streamCache, cacheKey, () => resolveSrc(creds, torrentUrl, false));
}
