...
// Recognised video container extensions - matches realdebrid.js
...
async function resolveStreams(apiKey: string, infoHash: string, magnetLink: string, userIp: string | undefined) {
  const cacheKey = `tb:meta:${scope(apiKey)}:${infoHash}`;
...
async function resolveStreamsFromTorrentFile(apiKey: string, torrentUrl: string, userIp: string | undefined) {
  const cacheKey = `tb:meta-turl:${scope(apiKey)}:${torrentUrl}`;
  const cached = await streamCache.get(cacheKey) as TbCacheMeta | undefined;
  if (cached) {
    return await buildFileResults(apiKey, cached.torrentId, cached.files, userIp);
  }
...
/**
 * Cache scope token unique to a user account. The cached torrent_id belongs to
 * one TorBox account, so it must never be served to a different user (whose
 * token couldn't resolve it). The API key is hashed so the raw secret never
 * appears in a cache key. IP isn't part of the scope - the CDN URL is re-minted
 * with the current user's IP on every requestdl call.
 */
