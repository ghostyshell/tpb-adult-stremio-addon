...
import { VIDEO_EXT } from './debridUtils';
import type { DebridFile } from '../types/debrid';
 ...
 *
 * ponytail: kept local (not the shared scopeKey) because it mixes in userIp,
 * and the resolve cache stays local (not cachedResolve) because it re-caches a
 * partial `existing` result on hits - neither matches the shared helpers.
 */
...
async function resolveStreams(apiKey: string, infoHash: string, magnetLink: string, userIp?: string): Promise<any[]> {
  ...
  const cached = await streamCache.get(cacheKey) as DebridFile[] | undefined;
...
async function addTorrentFile(apiKey: string, torrentUrl: string, userIp?: string) {
  // SSRF defense at the fetch boundary: every caller (and the prewarm path)
  // is now covered here too, so a future refactor that drops the call-site
  // check cannot re-open the server fetch to internal URLs.
  const { isSafeUrl } = require('../utils/safeUrl');
  const safe = isSafeUrl(torrentUrl);
  if (!safe.ok) throw new Error(`addTorrentFile refused unsafe url: ${safe.reason}`);
