...
import {
  ...
  parseVariants,
  rewriteM3u8Urls,
  stripchatAbsStreamUrl,
  variantLabel,
  withPkeyParams,
} from './stripchatHls';
import { parsePkeyFromMaster } from './stripchatKeys';
...
describe('parseBroadcastItem', () => {
  it('maps a live broadcasts item to CamData', () => {
    expect(parseBroadcastItem({
      streamName: '141350658',
      isLive: true,
      status: 'public',
    })).toEqual({
      streamName: '141350658',
  ...
  it('returns null when streamName is missing', () => {
    expect(parseBroadcastItem({ isLive: true, status: 'offline' })).toBeNull();
    expect(parseBroadcastItem(null)).toBeNull();
  });
});

describe('withPkeyParams', () => {
  it('adds psch=v2 and pkey to bare URLs', () => {
    expect(withPkeyParams('https://cdn/hls/v.m3u8', 'abc123'))
      .toBe('https://cdn/hls/v.m3u8?psch=v2&pkey=abc123');
  });

  it('adds psch=v2 when pkey is already present', () => {
    expect(withPkeyParams('https://cdn/hls/v.m3u8?pkey=abc123', 'abc123'))
      .toBe('https://cdn/hls/v.m3u8?pkey=abc123&psch=v2');
  });
});

describe('parsePkeyFromMaster', () => {
  it('reads the first v2 MOUFLON pkey', () => {
    const m3u8 = [
      '#EXTM3U',
      '#EXT-X-MOUFLON:PSCH:v2:1Dzcc6OjP73LKbtI',
      '#EXT-X-MOUFLON:PSCH:v2:Fq6m2TO2ZeBkRPm9',
    ].join('\n');
    expect(parsePkeyFromMaster(m3u8)).toBe('1Dzcc6OjP73LKbtI');
  });

  it('returns null when no MOUFLON tag exists', () => {
    expect(parsePkeyFromMaster('#EXTM3U\n#EXTINF:10,\n')).toBeNull();
  });
});
...
describe('variantLabel', () => {
  it.each([
    ...
    ['auto', 'Auto'],
    ['source', 'source'],
  ])('maps %s to %s', (resolution, label) => {
    expect(variantLabel(resolution)).toBe(label);
  });
});

describe('rewriteM3u8Urls', () => {
  it('rewrites segment URLs to absolute CDN URLs', () => {
    const input = [
      '#EXTM3U',
    ...
    expect(lines[3]).toBe('https://cdn.example.com/hls/seg1.ts');
    expect(lines[6]).toBe('#EXT-X-ENDLIST');
  });

  it('rewrites EXT-X-MAP init segment URIs to absolute URLs', () => {
    const input = '#EXT-X-MAP:URI="https://cdn.example.com/init.mp4"\n#EXTINF:2,\nseg.ts\n';
    const result = rewriteM3u8Urls(input, 'https://cdn.example.com/p.m3u8');
    expect(result).toContain('URI="https://cdn.example.com/init.mp4"');
    expect(result).not.toContain('/stripchat/seg');
  });
  ...
  it('resolves relative URLs against the playlist base', () => {
    const input = '#EXTINF:10,\nrelative/path/seg.ts\n';
    const result = rewriteM3u8Urls(input, 'https://cdn.example.com/hls/master.m3u8');
    expect(result).toContain('https://cdn.example.com/hls/relative/path/seg.ts');
    expect(result).not.toContain('/stripchat/seg');
  });
});

describe('stripchatAbsStreamUrl', () => {
  it('prefixes the addon base URL', () => {
    expect(stripchatAbsStreamUrl('https://addon.example', 'alice', '720x960')).toBe(
      'https://addon.example/stripchat/hls/alice/720x960',
    );
  });

  it('strips a trailing slash from the base URL', () => {
    expect(stripchatAbsStreamUrl('https://addon.example/', 'bob', '360x480')).toBe(
      'https://addon.example/stripchat/hls/bob/360x480',
    );
  });
});
