...
const { decodeItemId, decodeGroupId, encodeItemId, buildMagnet, qualityTag, detectQuality, encodeMediaFlowProxyUrl,
        fetchInfoHashFromTorrentUrl, isPornripsUrl, extractInfoHash } = require('../utils/torrent');
const { isSafeUrl } = require('../utils/safeUrl');
const { getTorrent } = require('../utils/torrentCache');
const { resolveDownloadUrl } = require('../services/pornrips');
const porntube = require('../services/porntube');
...
const EXTRA_INDEXER_WEBSITES = new Set(['knaben', 'bitsearch', 'xxxclub']);

const QUALITY_RANK: Record<string, number> = { '2160p': 3, '1080p': 2, '720p': 1 };

// Drop 480p globally; when the item came from a quality-scoped catalog, also
// drop streams whose resolution doesn't match that catalog.
...
// quality and users opt in knowing that; only the 480p floor still applies.
...
async function handleStream(req: Request, res: Response) {
  ...
  //   pt: PornTube - torrents (infoHash) → debrid + P2P, all providers supported.
  //   hs: Hentai   - direct episode URLs (no debrid).
  if (id.startsWith('pt:')) {
    const streams = await porntubeStreams(cfg, id, getActiveProvider(cfg), publicClientIp(req), debridOn).catch(() => []);
    return res.json({ streams: applyQualityFilter(streams, id) });
  }
  if (id.startsWith('hs:')) {
    const streams = await hentaiStreams(id).catch(() => []);
    return res.json({ streams: applyQualityFilter(streams, id) });
  }
  if (isHentaiEpisodeId(id)) {
    const streams = await hentaiEpisodeStreams(id).catch(() => []);
    return res.json({ streams: applyQualityFilter(streams, id) });
  }

  if (id.startsWith('porndb:')) {
    const streams = await porndbStreams(cfg, id, String(req.params['type']), getActiveProvider(cfg), publicClientIp(req), debridOn).catch(() => []);
    return res.json({ streams: applyQualityFilter(streams, id) });
  }

  if (id.startsWith('sc:')) {
    const streams = await stripchatStreams(cfg, id).catch(() => []);
  ...
  try {
    ...
    if (id.startsWith('jstrg:')) {
      ...
      const perMember = await Promise.all(records.map((r: Record<string, unknown>) =>
        resolveOneRecord(cfg, encodeItemId(r), provider, userIp, debridOn)));
      const flat = perMember.flat() as any[];
      flat.sort((a, b) => (QUALITY_RANK[detectQuality(b.name || '')] ?? 0) - (QUALITY_RANK[detectQuality(a.name || '')] ?? 0));
      return res.json({ streams: flat });
    }

    const streams = await resolveOneRecord(cfg, id, provider, userIp, debridOn);
    res.json({ streams });
  } catch (err: any) {
...
async function resolveOneRecord(cfg: any, id: string, provider: any, userIp: string | undefined, debridOn: boolean): Promise<object[]> {
  try {
    let streams: object[] = [];
    ...
    // Filter by quality: always drop 480p; when the item came from a quality-
    // scoped catalog, drop streams that don't match that catalog's resolution.
...
async function torrentStreams(id: string) {
  ...
  // PornRips catalog items carry only a detailUrl (no infoHash) - resolve it
  // lazily here. The resolver returns a magnet for most posts and a .torrent
  // URL for the rest; handle both.
  let resolvedMagnet = '';
  if (!infoHash && isPornripsItem(record.w || stored?.website, torrentUrl, detailUrl)) {
    const resolved = await resolvePornripsTorrentUrl(torrentUrl, detailUrl);
    if (resolved.startsWith('magnet:')) {
      resolvedMagnet = resolved;
      infoHash = (extractInfoHash(resolved) || '').toLowerCase();
    } else if (/\.torrent(\?|$)/i.test(resolved)) {
      torrentUrl = resolved;
      infoHash = await fetchInfoHashFromTorrentUrl(resolved).catch(() => '');
    }
  }

  if (!infoHash) return [];

  const magnet = resolvedMagnet || stored?.magnetLink || buildMagnet(infoHash, title);
...
/**
 * PornTube streams: the reference addon returns one torrent (infoHash) per
 * quality. For each, resolve through the configured debrid provider; fall back
 * to a P2P magnet stream when there's no debrid hit (or no provider). This is
 * how PornTube gains support for all our debrid providers.
 */
async function porntubeStreams(cfg: AddonConfig, id: string, provider: DebridProvider | null, userIp: string | undefined, debridOn: boolean) {
  const raw = await porntube.getStreams(id.slice(3)); // strip "pt:"
  const out = [];
  for (const s of raw) {
    ...
    let debridStreams = [];
    if (provider) {
      debridStreams = await debridStreamsForHash(cfg, provider, userIp, infoHash, title);
      out.push(...debridStreams);
    }
    // P2P fallback for this quality when debrid found nothing (or is off).
    if (debridStreams.length === 0 && !(cfg.hideP2P && debridOn)) {
      out.push(p2pStream(infoHash, title));
    }
  }
  return out;
}
 ...
 * edge resolves each hash through debrid (same path as PornTube).
 */
...
async function debridStreamsForHash(cfg: AddonConfig, provider: DebridProvider, userIp: string | undefined, infoHash: string, title: string) {
  ...
  try {
    const files = await provider.resolve(apiKey, infoHash, buildMagnet(infoHash, title), ipArg);
...
/**
 * Hentai streams: the series maps to one 'Porn' card, so flatten every episode's
 * direct video URLs into a single labeled stream list. No debrid (direct URLs).
 */
async function hentaiStreams(id: string) {
  const seriesId = id.slice(3); // strip "hs:"
  const meta = await hentai.getMeta(seriesId);
  const videos = (meta && Array.isArray(meta.videos) && meta.videos.length)
    ? meta.videos
    : [{ id: seriesId, episode: 1 }];

  const perEpisode = await Promise.all(videos.map(async (v: any) => ({
    v,
    streams: await hentai.getEpisodeStreams(v.id).catch(() => []),
  })));

  const out = [];
  const multi = videos.length > 1;
  for (const { v, streams } of perEpisode) {
    const label = v.episode ? `E${String(v.episode).padStart(2, '0')}` : (v.title || '');
    for (const st of streams) {
      if (!st || !st.url) continue;
      out.push({
        url:  st.url,
        name: multi && label ? `Hentai ${label}` : 'Hentai',
        description: [multi ? label : '', st.name, st.title].filter(Boolean).join(' · ') || undefined,
...
// Upstream HentaiStream uses bare ids like hmm-..., hse-..., htv-... for both
// series and individual episodes. Stremio asks for streams per video id (which
// is bare), so we must answer those ids too.
function isHentaiEpisodeId(id: string): boolean {
  return /^h(mm|se|tv)-/.test(id);
}

async function hentaiEpisodeStreams(id: string) {
  const streams = await hentai.getEpisodeStreams(id).catch(() => []);
  return streams
    .filter((st: any) => st && st.url)
    .map((st: any) => ({
      url: st.url,
      name: st.name || 'Hentai',
      description: [st.title, st.name].filter(Boolean).join(' · ') || undefined,
      behaviorHints: st.behaviorHints || { notWebReady: false },
    }));
}
...
async function streamsForCustomId(cfg: AddonConfig, id: string, provider: DebridProvider, userIp: string | undefined) {
  ...
  const isPr       = isPornripsItem(website, torrentUrl, detailUrl);
  ...
  // PornRips catalog items carry only a detailUrl - resolve it lazily. The
  // resolver yields a magnet for most posts; fall back to a .torrent URL (kept
  // for the file-upload path below) for the rest.
  if (isPr && !infoHash) {
    const resolved = await resolvePornripsTorrentUrl(torrentUrl, detailUrl);
    if (resolved.startsWith('magnet:')) {
      infoHash = (extractInfoHash(resolved) || '').toLowerCase();
      if (infoHash) {
        const streams = await tryDebridMagnet(infoHash, resolved);
        if (streams.length) return streams;
      }
    } else if (/\.torrent(\?|$)/i.test(resolved)) {
      torrentUrl = resolved;
      infoHash = await fetchInfoHashFromTorrentUrl(resolved).catch(() => '');
      if (infoHash) {
        const streams = await tryDebridMagnet(infoHash, buildMagnet(infoHash, title));
        if (streams.length) return streams;
      }
    }
  }
...
function isPornripsItem(website: unknown, torrentUrl: unknown, detailUrl: unknown): boolean {
  return website === 'pornrips' || isPornripsUrl(torrentUrl) || isPornripsUrl(detailUrl);
}

async function resolvePornripsTorrentUrl(torrentUrl: unknown, detailUrl: unknown): Promise<string> {
  let url = String(torrentUrl || detailUrl || '');
  if (!url) return '';

  // Block unsafe URLs before any server-side fetch.
  const safe = isSafeUrl(url);
  if (!safe.ok) {
    console.warn(`[stream] unsafe PornRips URL rejected: ${safe.reason} (${url.slice(0, 80)})`);
    return '';
  }

  if (/\.torrent(\?|$)/i.test(url) || url.startsWith('magnet:')) return url;

  const resolved = await resolveDownloadUrl(url);
  if (resolved.startsWith('magnet:')) return resolved;
  // Only return a real magnet/.torrent; never fall back to the original HTML
  // detail URL, which would fail debrid upload and is misleading.
  if (/\.torrent(\?|$)/i.test(resolved)) return resolved;
  return '';
}
