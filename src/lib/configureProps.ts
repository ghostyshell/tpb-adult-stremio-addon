...
import { getSourceStatuses, type NonLiveStatus } from './addonStatus';
import {
  ALL_CATALOG_BASES,
  DEBRID_TOKEN_UI,
...
export interface ConfigureProps {
  ...
  sourceStatuses: Record<string, NonLiveStatus>;
}

export async function getConfigureProps(): Promise<ConfigureProps> {
  ...
  // Default install ships Stripchat + PornRips enabled alongside HiddenBay, so a
  // fresh user sees those catalogs without opting in. Operator single-source
  // choices (e.g. ADULT_SOURCE=hentai) are still honored - only the default
  // 'piratebay' and 'all' installs get the extra sources on.
  if (adultSource === 'piratebay' || adultSource === 'all') {
    initialSources.pornrips = true;
    initialSources.stripchat = true;
  }
  ...
  return {
    ...
    initialSources,
    sourceStatuses: await getSourceStatuses(),
  };
}
