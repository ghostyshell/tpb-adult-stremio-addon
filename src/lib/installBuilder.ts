...
export function buildInstallInstances(req: Request): InstallResult {
  ...
  const piratebayOn = q.src_piratebay === '1' || (!pornripsOn && !hentaiOn && !sukebeiOn && !stripchatOn);
  ...
  const stripchatNames = stripchatOn
    ? STRIPCHAT_CATALOG_IDS.filter((id) => !stripchatDisabled.includes(id)).map((id) => `Stripchat · ${id.slice(3)}`)
    : [];
