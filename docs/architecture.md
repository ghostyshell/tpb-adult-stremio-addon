...
buildInstallResponse()
  ├── Determines enabled content sources (piratebay, pornrips, porntube, hentai)
...
buildManifest(cfg, baseUrl)
  ...
  ├── getPorntubeManifestCatalogs() / getHentaiManifestCatalogs() → proxied sources
...
metaRouter → handleMeta()
  ├── Proxied sources (pt:/hs:) → buildProxiedMeta()
...
streamRouter → handleStream()
  ├── Proxied: pt: → porntubeStreams() | hs: → hentaiStreams()
...
| `cat:pt:v1:` / `cat:hs:v1:` | PornTube / Hentai proxied catalog lists | 30 min |
...
ADULT_SOURCE env or user source selection
         ...
         ├── 'porntube'     → services/porntube.js (proxied reference addon)
