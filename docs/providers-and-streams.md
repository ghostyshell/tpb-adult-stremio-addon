...
The addon supports 14 debrid providers. Each is implemented as a standalone module in `src/services/`. All providers expose the same interface:
...
```
1. POST /magnet/upload      body: magnet=…  → { data.magnets[0].id }
2. GET  /magnet/status/{id} poll until status=Ready
3. GET  /magnet/files/{id}  → file list with link per video file
4. POST /link/unlock        body: link=<file_link>  → { data.link }
```
...
```
1. POST /transfer/add_magnet  auth: email:password  → { id }
2. GET  /folder?id=0          poll until file appears
3. Returns fetch_url from folder contents
```

### PikPak

PikPak uses a refresh-token authentication flow distinct from all other providers:

```
1. Parse pkKey:
   - "os.…"  → bare refresh token
   - base64 JSON { access_token, refresh_token } → extract both

2. POST /v1/auth/token   body: grant_type=refresh_token, client_id=…, client_secret=…, refresh_token=…
        └── returns { access_token, refresh_token }

3. POST /drive/v1/files  body: { kind, upload_type: "UPLOAD_TYPE_URL", url: { url: <magnet> } }
        └── returns { task.id }

4. GET  /drive/v1/tasks?type=offline_download  poll every 3 s (up to 180 s)
        └── task.phase = "PHASE_TYPE_COMPLETE" → file.id

5. GET  /drive/v1/files/{file.id}
        └── medias[0].link.url  ← playable stream URL
```

The `CLIENT_ID` and `CLIENT_SECRET` constants in `src/services/pikpak.js` are PikPak's public OAuth app credentials — they identify the app, not the user.
...
Proxied from a reference external addon. The reference addon returns `{ infoHash }` per quality variant. The stream route resolves each infoHash through the user's configured debrid provider, then falls back to P2P — giving PornTube content access to all 14 providers.
