...
| Export | Description |
|--------|-------------|
| `buildManifest(cfg, baseUrl)` | Async. Fetches extra studios from the backend KV, assembles catalog entries from all enabled sources (TPB, PornRips, PornTube, Hentai), applies `enabledCatalogs`/`disabledCatalogs` filtering, builds the manifest `id` and `name` with provider + group suffixes. |
...
| `buildProxiedCatalog(catalogId, opts)` | Calls the reference addon service (porntube.js / hentai.js), re-namespaces item IDs with `pt:` / `hs:` prefix. |
...
- `pt:`/`hs:` IDs: proxies to the reference addon's meta endpoint.
...
| `buildProxiedMeta(id)` | Fetch and reformat a reference-addon meta for a `pt:` or `hs:` item. |
...
- For `pt:` IDs: fetches the reference addon's stream list (infoHash per quality), then resolves each through the debrid provider.
...
| `porntubeStreams(cfg, id, …)` | Resolves each PornTube infoHash through debrid; falls back to P2P per quality. |
...
Each of the 14 debrid providers has its own module with the same interface:

| Module | Provider | Auth type |
|--------|----------|-----------|
| `realdebrid.js` | Real-Debrid | Bearer API key |
| `alldebrid.js` | AllDebrid | Bearer API key |
...
| `pikpak.js` | PikPak | refresh token (`os.…`) |
...
PikPak requires a distinct auth flow:

1. Accept `pkKey` as either a bare `os.…` refresh token or base64-encoded JSON `{ access_token, refresh_token }`.
2. Exchange the refresh token for an access token via `POST /v1/auth/token`.
3. Submit an offline download task (`POST /drive/v1/files`) with the magnet URL.
4. Poll `/drive/v1/tasks` every 3 s (up to 180 s) until the task completes.
5. Fetch the file metadata (`GET /drive/v1/files/{id}`) and return `medias[0].link.url`.

The `CLIENT_ID` and `CLIENT_SECRET` constants in this file are public PikPak app credentials, not user secrets.
...
### `porntube.js` / `hentai.js`

Thin HTTP clients for proxied external reference addons. They implement:
...
- `getStreams(id)` — returns stream entries (PornTube: infoHash per quality; Hentai: direct video URLs per episode).
...
Central registry of all 14 debrid providers. Each entry:
...
| `DEBRID_KEY_FIELDS` | Array of all config key names (`rdKey`, `adKey`, …). |
...
Utilities for the proxied external sources (PornTube, Hentai): catalog ID mapping between local IDs and the reference addon's IDs, manifest catalog entries.
...
Static JSON file listing available genre options for the external catalog sources (PornTube, Hentai). Avoids a live API call on every manifest build.
