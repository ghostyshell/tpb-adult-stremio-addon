...
Global skill: `~/.claude/skills/changelog/SKILL.md` (install: `sh ~/Code/scripts/install-changelog-skill.sh`). Repo shim: `docs/agents/skills/changelog.md`. Before committing product code, add bullets under `CHANGELOG.md` → `[Unreleased]` and stage the changelog. Pre-commit fails if `src/` changes land without a `CHANGELOG.md` diff. Refresh shims: `sh scripts/sync-agent-skills.sh`.

## Version bumps — bump-addon-version

Repo skill (canonical: `docs/agents/skills/bump-addon-version.md`; shims in `.claude/skills/`, `.opencode/skills/`, `.cursor/skills/`). Before committing `src/` changes, run `sh scripts/bump-addon-version.sh patch` or let pre-commit auto-bump. Never regress below `node scripts/addon-version.mjs history-max`. Refresh shims after editing the canonical skill: `sh scripts/sync-agent-skills.sh`.

The addon version also flows to the Go backend manifest: the Node edge stamps `ADDON_VERSION` onto the proxied manifest in `src/utils/stremioGo.ts`, so Stremio sees the current version on edge deploy. Also bump the Go `addonVersion` const in `torrent-search-go/internal/stremio/manifest.go` (or set its `ADDON_VERSION` deploy env) so direct-backend hits stay aligned. `package.json` remains the single source of truth.
