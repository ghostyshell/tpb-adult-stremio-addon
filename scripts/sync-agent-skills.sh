...
#
# NOTE: only single-file skills (one docs/agents/skills/<slug>.md) are shims.
# Multi-file skills are vendored as full trees and are EXEMPT from this script:
#   - refactor  (docs/agents/skills/refactor/ : SKILL.md + phases/ + references/)
#              live copies: .claude/skills/refactor/ + .opencode/skills/refactor/
#              agent: docs/agents/code-validator.md + .claude/agents/code-validator.md
#              Keep the three trees identical by hand; this script does not touch them.
set -e
