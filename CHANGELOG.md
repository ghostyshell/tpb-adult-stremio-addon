...
### Changed
- **Account modal** - replaced raw auth-key input with Stremio email/password login; credentials go directly to Stremio's API (`api.strem.io`), only the returned `authKey` is used as the profile identifier. OAuth users (Google/Facebook) can paste their key via the "Signed up with Google/Facebook?" toggle.
